...
Tiberium Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zTiberium spends, automint calculation adjustments, and other various updates/fixes.

zTiberium spending requires this update.
...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each zTiberium mint operation (zTiberium spends which re-mint their change are also included in this). This functionality is controlled by the `-backupztiberium` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupztiberium=0` at the command-line when starting the client, or add `backupztiberium=0` to their `Tiberium.conf` file.

zTiberium Automint Calculations
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zTiberium mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

zTiberium Spending Fix
---------------------
The size of zTiberium spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

zTiberium Transaction Recovery
---------------------
Due to the aforementioned issue with zTiberium spending, users may find that their attempted spends are now conflicted and zTiberium balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
- #279 `e734010` Add -backupztiberium startup flag. (presstab)
